from django.urls import path

from . import views


urlpatterns = [
path("employees_home", views.employees_home, name='employees_home'),
path("employees_page/update_pending_action_certification", views.update_employee_pending_action_certification, name='update_employee_pending_action_certification'),
path("my_page/update_review_pending_action_certification", views.update_review_pending_action_certification, name='update_review_pending_action_certification'),
path("employees_page/<id>", views.employees_page, name='employees_page'),
path("subcontractor_employee_page/<int:id>", views.subcontractor_employee_page, name='subcontractor_employee_page'),
path("training", views.training, name='training'),
path("my_page", views.my_page, name='my_page'),
path("certifications/<id>", views.certifications, name='certifications'),
path("certifications/<int:certification_id>/files/<path:filename>/", views.certification_file, name='certification_file'),
path("subcontractor_certifications", views.subcontractor_certifications, name='subcontractor_certifications'),
path("certification_categories", views.certification_categories, name='certification_categories'),
path("new_certification", views.new_certification, name='new_certification'),
path("add_new_employee", views.add_new_employee, name='add_new_employee'),
path("assessments/<id>", views.assessments, name='assessments'),
path("production_reports/<id>", views.production_reports, name='production_reports'),
path("classes/<id>", views.classes, name='classes'),
path("new_class", views.new_class, name='new_class'),
path("exams/<id>", views.exams, name='exams'),
path("new_exam", views.new_exam, name='new_exam'),
path("mentorships/<id>", views.mentorships, name='mentorships'),
path("new_mentorship", views.new_mentorship, name='new_mentorship'),
path("new_assessment/<id>", views.new_assessment, name='new_assessment'),
path("new_production_report/<jobnumber>", views.new_production_report, name='new_production_report'),
path("write_ups/<id>", views.write_ups, name='write_ups'),
path("write_ups_new", views.write_ups_new, name='write_ups_new'),
path("employee_notes/<employee>", views.employee_notes, name='employee_notes'),
path("daily_reports/<id>", views.daily_reports, name='daily_reports'),
path('safety_home', views.safety_home, name='safety_home'),
path('toolbox_talks_master', views.toolbox_talks_master, name='toolbox_talks_master'),
path('scheduled_toolbox_talks', views.scheduled_toolbox_talks, name='scheduled_toolbox_talks'),
path('delete_old_incomplete_toolbox_talks', views.delete_old_incomplete_toolbox_talks, name='delete_old_incomplete_toolbox_talks'),
path('respirator_clearance_section1', views.respirator_clearance_section1, name='respirator_clearance_section1'),
path('respirator_clearance_section2', views.respirator_clearance_section2, name='respirator_clearance_section2'),
path('respirator_clearance_section3', views.respirator_clearance_section3, name='respirator_clearance_section3'),
path('respirator_clearance_section4', views.respirator_clearance_section4, name='respirator_clearance_section4'),
path('respirator_clearance_section5', views.respirator_clearance_section5, name='respirator_clearance_section5'),
path('respirator_clearance_section6', views.respirator_clearance_section6, name='respirator_clearance_section6'),
path('respirator_clearance_section0', views.respirator_clearance_section0, name='respirator_clearance_section0'),
path('respirator_clearance_base', views.respirator_clearance_base, name='respirator_clearance_base'),
path('respirator_clearance_completed/<respirator_id>', views.respirator_clearance_completed, name='respirator_clearance_completed'),
path('toolbox_talks_by_employee', views.toolbox_talks_by_employee, name='toolbox_talks_by_employee'),
path("view_respirator_certification/<id>", views.view_respirator_certification, name='view_respirator_certification'),
path("delete_employee/<id>", views.delete_employee, name='delete_employee'),
path("upload_employees", views.upload_employees, name="upload_employees"),
path("toolbox_file/<int:scheduled_id>/<str:language>/",
     views.toolbox_file,
     name="toolbox_file"),
path("group_toolbox_file/<int:scheduled_id>/<str:language>/",
     views.group_toolbox_file,
     name="group_toolbox_file"),
path("toolbox_file_for_employee/<int:employee_id>/<int:scheduled_id>/<str:language>/",
     views.toolbox_file_for_employee,
     name="toolbox_file_for_employee"),
path('toolbox-talk/assign/', views.toolbox_talk_assign, name='toolbox_talk_assign'),
path('ajax/subcontractor-employees/', views.ajax_subcontractor_employees, name='ajax_subcontractor_employees'),
path('ajax/subcontractor-jobs/', views.ajax_subcontractor_jobs, name='ajax_subcontractor_jobs'),
path('employees/participation_ajax', views.participation_ajax, name='participation_ajax'),
path('upload_toolbox_file/', views.upload_toolbox_file, name='upload_toolbox_file'),
path('get_scheduled_toolbox_folder/', views.get_scheduled_toolbox_folder, name='get_scheduled_toolbox_folder'),
path('scheduled_toolbox_report/<int:scheduled_id>/', views.scheduled_toolbox_report, name='scheduled_toolbox_report'),
path('ajax/job-sorted-employees/', views.ajax_job_sorted_employees, name='ajax_job_sorted_employees'),
path('delete_scheduled_toolbox_talk/', views.delete_scheduled_toolbox_talk, name='delete_scheduled_toolbox_talk'),
path('toolbox_talks_by_employee/modal/<str:person_type>/<int:person_id>/',views.toolbox_talks_by_employee_modal,name='toolbox_talks_by_employee_modal'),
path('employee_edit/<int:id>/', views.employee_edit, name='employee_edit'),
path('reinstate_employee/<int:id>/',views.reinstate_employee,name='reinstate_employee'),
path('reinstate_subcontractor_employee/<int:id>/',views.reinstate_subcontractor_employee,name='reinstate_subcontractor_employee'),
path(
    'toolbox-talks-master/upload-file/',
    views.upload_master_toolbox_file,
    name='upload_master_toolbox_file'
),
path(
    'ajax/job-subcontractors/',
    views.ajax_job_subcontractors,
    name='ajax_job_subcontractors'
),
path(
    'scheduled-toolbox-talks/<int:scheduled_id>/edit/',
    views.scheduled_toolbox_talk_edit,
    name='scheduled_toolbox_talk_edit'
),
path(
    'subcontractors/<int:sub_id>/toggle-delegation/',
    views.toggle_subcontractor_delegation,
    name='toggle_subcontractor_delegation'
),
path(
    "ajax/check-toolbox-can-complete/",
    views.ajax_check_toolbox_can_complete,
    name="ajax_check_toolbox_can_complete"
),
]
