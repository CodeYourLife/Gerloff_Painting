...
def submittals_home(request):
    send_data = {}
    send_data['submittals'] = Submittals.objects.filter(job_number__status="Open").order_by('job_number',
                                                                                            'submittal_number')
    ...
    if request.method == 'POST':
        if 'job_select' in request.POST:
            ...
            if Wallcovering.objects.filter(job_number=request.POST['select_job']).exists():
                wallcovering = Wallcovering.objects.filter(job_number=request.POST['select_job']).values('id', 'code',
            ...
            if Submittals.objects.filter(job_number=request.POST['select_job']).exists():
                submittal = Submittals.objects.filter(job_number=request.POST['select_job']).order_by(
                    'submittal_number').last()
        ...
        if 'add_submittal' in request.POST:
            if 'other_item' in request.POST or 'wc_item' in request.POST:
                submittal = Submittals.objects.create(
                    ...
                    description=request.POST['description'], submittal_number=request.POST['submittal_number'],
                    date_sent=date.today())
                SubmittalNotes.objects.create(submittal=submittal, date=date.today(),
                                              user=Employees.objects.get(user=request.user),
                                              note="Sent! - " + request.POST['new_note'])
                ...
                if 'wc_item' in request.POST:
                    ...
                    for x in range(0, len(items)):
                        SubmittalItems.objects.create(submittal=submittal, description=items[x], quantity=quantity[x],
                                                      wallcovering_id=Wallcovering.objects.get(
                                                          id=request.POST['select_wc']), notes=notes[x])
